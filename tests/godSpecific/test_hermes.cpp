...
TEST(HermesTests, cannot_move_up_while_using_power) {
    ...
    Moves::Move move = Moves::create_move(0, 3, 4, Constants::God::HERMES);
...
TEST(HermesTests, can_move_on_h1) {
    ...
    Moves::Move move = Moves::create_move(0, 2, 3, Constants::God::HERMES);
...
TEST(HermesTests, to_text_for_stand_still_move) {
    ...
    Moves::Move hermes_move = Moves::create_move(6, 10, 6, Constants::God::HERMES);

    // Generate the text representation of the move.
    std::string move_text = Moves::move_to_text(board, hermes_move);
