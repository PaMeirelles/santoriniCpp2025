...
TEST(AthenaTests, athena_power_works) {
    ...
    Moves::AthenaMove move_athena(0, 1, 5); // From 0, to 1, build at 5
    EXPECT_TRUE(board.move_is_valid(move_athena)) << "Athena move should be valid";
    ...
    Moves::ApolloMove move_apollo(3, 8, 2); // From 3, to 8, build at 2
    EXPECT_FALSE(board.move_is_valid(move_apollo)) << "Apollo's upward move should be invalid due to Athena's power";
}
//----------------------------------------------------------------------------------------------------------------------
TEST(AthenaTests, opponent_generated_moves_do_not_climb_after_athena_up) {
    ...
    Moves::AthenaMove move_athena(0, 1, 5); // Move 0->1, build 5
    EXPECT_TRUE(board.move_is_valid(move_athena)) << "Athena move should be valid";
    board.make_move(move_athena);
    ...
    bool all_moves_are_not_upward = std::all_of(blue_moves.begin(), blue_moves.end(),
        [&](const std::unique_ptr<Moves::Move>& move) {
           ...
           sq_i final = move->final_sq();
            // In C++, we need to access _blocks directly for the board state.
            return board._blocks[final] <= board._blocks[origin];
        }
    );
...
TEST(AthenaTests, athena_opponent_no_move_loses) {
    ...
    Moves::AthenaMove move_athena(3, 2, 8);
    EXPECT_TRUE(board.move_is_valid(move_athena)) << "Athena's initial move should be valid";
    board.make_move(move_athena);
    ...
    EXPECT_EQ(board.check_state(), -1) << "Gray should lose immediately because Athena's power prevents all valid moves."; // 1 => Gray wins (because Blue causes Gray to lose)
}
